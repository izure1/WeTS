...
export default class VueComponent extends Vue {
    ...
    private static readonly SampleSound = new Howl({
        src: 'data:audio/wav;base64,UklGRjIAAABXQVZFZm10IBIAAAABAAEAQB8AAEAfAAABAAgAAABmYWN0BAAAAAAAAABkYXRhAAAAAA==',
        format: 'audio/wav'
    })
    private audio: Promise<Howl> = Promise.resolve(VueComponent.SampleSound)
    ...
    /**
     * @description         View 인스턴스의 Audio 컴포넌트 설정으로부터 오디오 설정을 반영합니다.
     */
    private async setAudioProperty(): Promise<void> {
        console.log(2)
        const audio: Howl = await this.audio
        console.log(2.01)
        if (!audio)
            return
    ...
    private observeAudioPosition(): void {
        this.intervalIndex = setInterval(async (): Promise<void> => {
    ...
    @Watch('body.component.audio', { deep: true })
    private onChangeAudioComponent(): void {
        console.log(3)
        this.setAudioProperty()
    }

...
class PairsManager {
     ...
     */
    static create(...pairLists: string[][]): Pairs {
        const pairs: Pairs = []
     ...
     */
    static get(pairs: Pairs, u: string): Pair {
        for (const pair of pairs) {
     ...
     */
    static has(pairs: Pairs, u: string): boolean {
     ...
     */
    static delete(pairs: Pairs, u: string): Pair | null {
        const t: Pair = PairsManager.get(pairs, u)
        if (!t) return null
        else {
     ...
     */
    static drop(pairs: Pairs, u: string): void {
        let i: number = pairs.length
        while (i--) {
     ...
     */
    static getAnd(pairs: Pairs, ...keys: string[]): Pair {
        const result: Set<string> = new Set
        for (const u of [...keys]) {
            const keys: Pair = PairsManager.get(pairs, u)
            if (keys.length)
     ...
     */
    static getOr(pairs: Pairs, ...keys: string[]): Pair {
        const result: Set<string> = new Set
        for (const u of [...keys]) {
            const keys: Pair = PairsManager.get(pairs, u)
            for (const key of keys) result.add(key)
